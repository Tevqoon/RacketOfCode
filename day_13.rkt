...
(define-values (numbers folds)
  (let*-values ([(numbers folds) (splitf-at (open-day 13) number?)])
    (values (list->set (pair-up numbers))
...
(define (make-paper numbers)
  (let-values ([(x0 y0) (apply values (map (λ(x) (+ 1 x (modulo x 2))) (apply map max (set->list numbers))))])
...
(define (printer paper)
  (for ([line paper]) (for-each (λ(c) (display (if c "o" " "))) line) (displayln "")))

(submit 1 (count identity (flatten (fold (car folds) (make-paper numbers)))) #f)
(printer (foldl fold (make-paper numbers) folds))
